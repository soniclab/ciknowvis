 ...
 */
public class MultipleDragControl extends ControlAdapter implements TableListener {

    private VisualItem activeItem;
    protected String action;
    protected Point2D down = new Point2D.Double();
    protected Point2D temp = new Point2D.Double();
    protected boolean dragged,resetItem; // wasFixed,
   // private boolean fixOnMouseOver = true;
     ...
     * is dragged.
     */
    public MultipleDragControl(GraphView gv) {
       this.gv = gv;
    }
     ...
     */
    public MultipleDragControl(boolean repaint) {
     ...
     */
  /*  public MultipleDragControl(boolean repaint, boolean fixOnMouseOver) {
        this.repaint = repaint;
     ...
     */
  /*  public MultipleDragControl(String action) {
        this.repaint = false;
        this.action = action;
    }
    */
     ...
     *                       fixed (made stationary) when the mouse pointer is over an item.
     */
   /* public MultipleDragControl(String action, boolean fixOnMouseOver) {
        this.repaint = false;
        this.fixOnMouseOver = fixOnMouseOver;
        this.action = action;
    } */
 ...
 /*   public void setFixPositionOnMouseOver(boolean s) {
  ...
  /*  public void itemEntered(VisualItem item, MouseEvent e) {
     ...
     */
    // extebds the original method in DragControl class to support multiple items drag.
    public void itemDragged(VisualItem item, MouseEvent e) {
        ...
        TupleSet search = vis.getGroup("_search_");

        Iterator searchit = search.tuples();
        while (searchit.hasNext()) {
            focusSet.addTuple((Tuple) searchit.next());
        }
        ...
        if (focusSet.getTupleCount() > 0) {
            Iterator iter = focusSet.tuples();

            while (iter.hasNext()) {
                VisualItem it = (VisualItem) iter.next();
                double x = it.getX();
                double y = it.getY();
     ...
     protected static void move(VisualItem item, double dx, double dy) {
        if (item instanceof AggregateItem) {
            ...
            // move items inside AggregateItem
            double x = item.getX();
            double y = item.getY();
            item.setStartX(x);
            item.setStartY(y);
