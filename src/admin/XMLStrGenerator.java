...
public class XMLStrGenerator {
     ...
     */
    public XMLStrGenerator(StringBuffer out) {
        this(out, 2);
    }
     ...
     */
    public XMLStrGenerator(StringBuffer out, int tabLength) {
        m_out = out;
        m_tab = 2;
    }
            ...
            int nattr, boolean close)
    {
        ...
        for ( int i=0; i<nattr; ++i ) {
            m_out.append(' ');
            m_out.append(names[i]);
            m_out.append('=');
            m_out.append('\"');
            escapeString(values[i]);
            m_out.append('\"');
        }
        if ( close ) m_out.append('/');
        m_out.append('>');
        println();
            ...
            int nattr, boolean close)
    {
        ...
        for ( int i=0; i<nattr; ++i ) {
            m_out.append(' ');
            m_out.append((String)names.get(i));
            m_out.append('=');
            m_out.append('\"');
    ...
    {
        spacing();
        m_out.append('<'); m_out.append(tag); m_out.append(' ');
        m_out.append(name); m_out.append('=');
        m_out.append('\"'); escapeString(value); m_out.append('\"');
        m_out.append('>');
        escapeString(content);
        m_out.append('<'); m_out.append('/'); m_out.append(tag); m_out.append('>');
        println();
    }
    ...
    protected void escapeString(String str) {
        ...
        for (int i = 0; i < len; ++i) {
                 ...
                 || (c > UPPER_RANGE) )
            {
                ...
                m_out.append("&#");
                m_out.append(Integer.toString(c));
                m_out.append(';');
            } else {
                boolean valid = true;
